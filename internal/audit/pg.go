...
func (s *PGStore) List(ctx context.Context, f Filter, limit, offset int) ([]Rec, error) {
	rows, err := db.New(s.pool).ListLLMCalls(ctx, db.ListLLMCallsParams{
		LabelFilter: f.Label, CorrelationFilter: f.CorrelationID,
		ErrorsOnly: f.ErrorsOnly, Lim: int32(limit), Off: int32(offset),
	})
	if err != nil {
		return nil, err
	}
	...
	for _, r := range rows {
		out = append(out, Rec{
			ID: r.ID, TS: r.Ts, Label: r.Label, CorrelationID: r.CorrelationID,
			Model: r.Model, Provider: r.Provider,
