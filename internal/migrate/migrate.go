...
// cmd/radio-lab-migrate (the Argo PreSync Job), internal/testutil, and
...
const lockKey int64 = 5127830194726004411

// minSafeDownVersion guards migration 00001's Down (DROP ledger_line). The
// ledger is regenerable bench spend, not irreplaceable data, but reversing it
// still wipes recorded totals — require -force-destructive, same contract as
// the sibling service.
const minSafeDownVersion int64 = 0
...
// current version is already <= minSafeDownVersion+1): 001's Down drops
// ledger_line, destroying every recorded ledger line in production. Pass
// force only for a deliberate teardown of a throwaway/dev database.
func Down(ctx context.Context, url string, force bool) (string, error) {
	...
	if force {
		...
		if version <= minSafeDownVersion+1 {
			return "", fmt.Errorf("refusing to reverse migration 001 (current version=%d): its Down DROPS ledger_line — every recorded ledger line in production; pass -force-destructive only for a throwaway/dev database", version)
		}
